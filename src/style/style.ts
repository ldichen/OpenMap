...
class Style extends Evented<MapEvents> {
  ...
  constructor(map: MapboxMap, options: StyleOptions = {}) {
    ...
    this._configDependentLayers = options.configDependentLayers
      ? options.configDependentLayers
      : new Set();
  ...
  _getGlobalId(
    loadedStyle?: StyleSpecification | string | null
  ): string | null {
    if (!loadedStyle) {
      return null;
    }
  ...
  _diffStyle(
    ...
    onStarted: (err: Error | null, isUpdateNeeded: boolean) => void,
    onFinished?: () => void
  ) {
    ...
    const handleStyle = (
      ...
      callback: (err: Error | null, isUpdateNeeded: boolean) => void
    ) => {
      try {
  ...
  loadURL(
    ...
    this._request = getJSON(
      request,
      (error?: Error | null, json?: any | null) => {
        this._request = null;
        if (error) {
          this.fire(new ErrorEvent(error));
        } else if (json) {
  ...
  _loadImports(
    ...
    for (const importSpec of imports) {
      ...
      if (beforeId) {
        const beforeIndex = this.fragments.findIndex(
          ({ id }) => id === beforeId
        );

        assert(
          beforeIndex !== -1,
          `Import with id "${beforeId}" does not exist on this map`
        );
  ...
  _isInternalStyle(json: StyleSpecification): boolean {
    return (
      this.isRootStyle() &&
      (json.fragment || (!!json.schema && json.fragment !== false))
    );
  }
  ...
  _load(json: StyleSpecification, validate: boolean) {
    ...
    const proceedWithStyleLoad = () => {
      ...
      for (const layer of layers) {
        const styleLayer = createStyleLayer(
          layer,
          this.scope,
          this._styleColorTheme.lut,
          this.options
        );
        if (styleLayer.isConfigDependent)
        ...
        const shadowsEnabled =
          !!this.directionalLight && this.directionalLight.shadowsEnabled();
      ...
      if (json.imports) {
        this._loadImports(json.imports, validate).then(() => {
          this._reloadImports();
          this.fire(
            new Event(isRootStyle ? "style.load" : "style.import.load")
          );
        });
      } else {
        this._reloadImports();
  ...
  mergeAll() {
    ...
    this.forEachFragmentStyle((style: Style) => {
      ...
      if (style.light != null) light = style.light;
      ...
      if (style.stylesheet.fog && style.fog != null) fog = style.fog;

      if (style.stylesheet.camera != null) camera = style.stylesheet.camera;
  ...
  _prioritizeTerrain(
    ...
    const prevIsDeffered =
      prevTerrain && prevTerrain.drapeRenderMode === DrapeRenderMode.deferred;
    const nextIsDeffered =
      nextTerrain && nextTerrain.drapeRenderMode === DrapeRenderMode.deferred;
    ...
    if (nextTerrain != null) {
      const nextIsElevated =
        nextTerrain && nextTerrain.drapeRenderMode === DrapeRenderMode.elevated;
  ...
  _evaluateColorThemeData(theme: ColorThemeSpecification): string | null {
    ...
    const properties = evaluateColorThemeProperties(
      this.scope,
      theme,
      this.options
    );
  ...
  get order(): Array<string> {
    if (this.terrain) {
      assert(
        this._drapedFirstOrder.length === this._mergedOrder.length,
        "drapedFirstOrder doesn't match order"
      );
  ...
  _checkLayer(layerId: string): StyleLayer | null | undefined {
    ...
    if (!layer) {
      this.fire(
        new ErrorEvent(
          new Error(`The layer '${layerId}' does not exist in the map's style.`)
        )
      );
      return;
    }
    ...
    for (const layerId of this._mergedOrder) {
      ...
      if (!layer.isHidden(parameters.zoom)) {
        ...
        if (sourceCache) {
          ...
          sourceCache.tileCoverLift = Math.max(
            sourceCache.tileCoverLift,
            layer.tileCoverLift()
          );
        }
      }
      ...
      if (!this._precompileDone && this._shouldPrecompile) {
        for (
          let i = layer.minzoom || DEFAULT_MIN_ZOOM;
          i < (layer.maxzoom || DEFAULT_MAX_ZOOM);
          i++
        ) {
          ...
          if (painter) {
            ...
            for (const programId of programIds) {
              const params = layer.getDefaultProgramParams(
                programId,
                parameters.zoom,
                this._styleColorTheme.lut
              );
  ...
  setState(nextState: StyleSpecification, onFinish?: () => void): boolean {
    ...
    if (unimplementedOps.length > 0) {
      throw new Error(
        `Unimplemented: ${unimplementedOps.map((op) => op.command).join(", ")}.`
      );
    }
  ...
  addImage(id: string, image: StyleImage): this {
    if (this.getImage(id)) {
      return this.fire(
        new ErrorEvent(new Error("An image with this name already exists."))
      );
    }
  ...
  removeModel(id: string): this {
    if (!this.hasModel(id)) {
      return this.fire(
        new ErrorEvent(new Error("No model with this ID exists."))
      );
    }
  ...
  addSource(
    ...
    this._checkLoaded();
    console.log("addSource", id, source);
    ...
    if (
      shouldValidate &&
      this._validate(validateSource, `sources.${id}`, source, null, options)
    )
      return;

    if (this.map && this.map._collectResourceTiming)
      (source as any).collectResourceTiming = true;
    ...
    const addSourceCache = (onlySymbols: boolean) => {
      const sourceCacheId =
        (onlySymbols ? "symbol:" : "other:") + sourceInstance.id;
      const sourceCacheFQID = makeFQID(sourceCacheId, this.scope);
      const sourceCache = (this._sourceCaches[sourceCacheId] = new SourceCache(
        sourceCacheFQID,
        sourceInstance,
        onlySymbols
      ));
  ...
  setGeoJSONSourceData(id: string, data: GeoJSON.GeoJSON | string) {
    ...
    assert(
      this.getOwnSource(id) !== undefined,
  ...
  setLights(lights?: Array<LightsSpecification> | null) {
      ...
      switch (light.type) {
        case "ambient":
          if (this.ambientLight) {
            ...
            this.ambientLight = new Lights<Ambient>(
              light,
              ambientProps,
              this.scope,
              this.options
        ...
        case "directional":
          if (this.directionalLight) {
            ...
            this.directionalLight = new Lights<Directional>(
              light,
              directionalProps,
              this.scope,
              this.options
    ...
    const evaluationParameters = new EvaluationParameters(
      this.z || 0,
  ...
  calculateLightsBrightness(): number | null | undefined {
    ...
    const relativeLuminance = (color: [number, number, number, number]) => {
      const r =
        color[0] <= 0.03928
          ? color[0] / 12.92
          : Math.pow((color[0] + 0.055) / 1.055, 2.4);
      const g =
        color[1] <= 0.03928
          ? color[1] / 12.92
          : Math.pow((color[1] + 0.055) / 1.055, 2.4);
      const b =
        color[2] <= 0.03928
          ? color[2] / 12.92
          : Math.pow((color[2] + 0.055) / 1.055, 2.4);
      return 0.2126 * r + 0.7152 * g + 0.0722 * b;
    };

    const directionalColor = directional.properties
      .get("color")
      .toRenderColor(null)
      .toArray01();
    ...
    const sphericalDirection = cartesianPositionToSpherical(
      direction.x,
      direction.y,
      direction.z
    );
    ...
    const directionalBrightness =
      relativeLuminance(directionalColor) *
      directionalIntensity *
      polarIntensity;

    const ambientColor = ambient.properties
      .get("color")
      .toRenderColor(null)
      .toArray01();
    ...
    const ambientBrightness =
      relativeLuminance(ambientColor) * ambientIntensity;
  ...
  getConfigProperty(fragmentId: string, key: string): unknown {
    ...
    const expression = expressions
      ? expressions.value || expressions.default
      : null;
  ...
  setConfigProperty(fragmentId: string, key: string, value: unknown) {
    ...
    if (!defaultExpression) {
      this.fire(
        new ErrorEvent(
          new Error(
            `No schema defined for the config option "${key}" in the "${fragmentId}" fragment.`
          )
        )
      );
      return;
  ...
  updateConfig(
    config?: ConfigSpecification | null,
    schema?: SchemaSpecification | null
  ) {
    this._config = config;
    ...
    if (!schema) {
      this.fire(
        new ErrorEvent(
          new Error(`Attempting to set config for a style without schema.`)
        )
      );
      return;
    }
    ...
    for (const id in schema) {
      ...
      if (defaultExpression) {
        ...
        this.fire(
          new ErrorEvent(
            new Error(`No schema defined for config option "${id}".`)
          )
        );
      }
    }
  ...
  updateConfigDependencies() {
    ...
    this.forEachFragmentStyle((style: Style) => {
      if (style._styleColorTheme.colorTheme) {
        const data = style._evaluateColorThemeData(
          style._styleColorTheme.colorTheme
        );
        if (
          !style._styleColorTheme.lut ||
          (style._styleColorTheme.lut &&
            data !== style._styleColorTheme.lut.data)
        ) {
  ...
  addLayer(
    ...
    if (layerObject.type === "custom") {
      ...
      layer = createStyleLayer(
        layerObject as LayerSpecification,
        this.scope,
        this._styleColorTheme.lut,
        this.options
      );
    ...
    if (before) {
      ...
      if (layer.slot === beforeLayer.slot) index = beforeIndex;
        ...
        warnOnce(
          `Layer with id "${before}" has a different slot. Layers can only be rearranged within the same slot.`
        );
    }
  ...
  removeLayer(id: string) {
    ...
    if (sourceCache && sourceCache.castsShadows) {
      ...
      for (const key in this._layers) {
        if (
          this._layers[key].source === layer.source &&
          this._layers[key].canCastShadows()
        ) {
  ...
  setLayoutProperty(
    ...
    if (
      ...
      const errors = emitValidationErrors(
        layer,
        validateLayoutProperty.call(validateStyle, {
          key,
          layerType: layer.type,
          objectKey: name,
          value,
  ...
  getLayoutProperty(
    layerId: string,
    name: string
  ): PropertyValueSpecification<unknown> | null | undefined {
    const layer = this._checkLayer(layerId);
    if (!layer) return;
  ...
  setPaintProperty(
    ...
    if (
      ...
      const errors = emitValidationErrors(
        layer,
        validatePaintProperty.call(validateStyle, {
          key,
          layerType: layer.type,
          objectKey: name,
          value,
  ...
  getPaintProperty(
    layerId: string,
    name: string
  ): void | TransitionSpecification | PropertyValueSpecification<unknown> {
    const layer = this._checkLayer(layerId);
    if (!layer) return;
  ...
  setFeatureState(
    target: FeatureSelector | GeoJSONFeature,
    state: FeatureState
  ) {
    this._checkLoaded();
    ...
    if (sourceType === "geojson" && sourceLayer) {
      this.fire(
        new ErrorEvent(
          new Error(`GeoJSON sources cannot have a sourceLayer parameter.`)
        )
      );
      return;
    }
    ...
    if (target.id === undefined) {
      this.fire(
        new ErrorEvent(new Error(`The feature id parameter must be provided.`))
      );
    }
  ...
  removeFeatureState(
    target:
      | (Omit<FeatureSelector, "id"> & { id?: FeatureSelector["id"] })
      | GeoJSONFeature,
    key?: string
  ) {
    this._checkLoaded();
  ...
  setTransition(transition?: TransitionSpecification | null): Style {
    this.stylesheet.transition = extend(
      {},
  ...
  serialize(): StyleSpecification {
    ...
    const scopedTerrain =
      terrain && this.terrain && this.terrain.scope === this.scope
        ? terrain
        : this.stylesheet.terrain;
  ...
  _flattenAndSortRenderedFeatures(
    sourceResults: Array<QueryResult>
    ...
    const isLayer3D = (layerId: string) =>
      this._mergedLayers[layerId].type === "fill-extrusion" ||
      this._mergedLayers[layerId].type === "model";
  ...
  queryRenderedFeatures(
    ...
    if (params && params.layers) {
      if (!Array.isArray(params.layers)) {
        this.fire(
          new ErrorEvent(new Error("parameters.layers must be an Array."))
        );
        return [];
      }
  ...
  setTerrain(
    terrainOptions?: TerrainSpecification | null,
    ...
    if (!terrainOptions) {
      ...
      if (
        ...
        drapeRenderMode === DrapeRenderMode.deferred
      ) {
        delete this.terrain;
      }
    ...
    if (drapeRenderMode === DrapeRenderMode.elevated) {
      ...
      if (this.terrain && isUpdating) {
        ...
        const fragmentStyle = this.terrain
    ...
    if (
      !this.terrain ||
      (this.terrain.scope !== this.scope && !isUpdating) ||
      ...
      for (const name of Object.keys(styleSpec.terrain)) {
        ...
        if (
          ...
          !!styleSpec.terrain[name].default
        ) {
  ...
  _getTransitionParameters(
    transition?: TransitionSpecification | null
  ): TransitionParameters {
    return {
      now: browser.now(),
  ...
  updateSources(transform: Transform) {
    ...
    for (const id in this._mergedSourceCaches) {
      this._mergedSourceCaches[id].update(
        transform,
        undefined,
        undefined,
        lightDirection
  ...
  _updatePlacement(
    ...
    for (const layerId of this._mergedOrder) {
      ...
      if (!sourceTiles) {
        ...
        sourceTiles = layerTiles[sourceId] = tiles.sort(
          (a, b) =>
    ...
    if (
      ...
      const fogState =
        this.fog && transform.projection.supportsFog ? this.fog.state : null;
    ...
    if (placementCommitted || symbolBucketsChanged) {
      ...
      for (let i = 0; i < this._mergedOrder.length; i++) {
        ...
        const styleLayer = this._mergedLayers[layerId];
        if (styleLayer.type !== "symbol") continue;
        const checkAgainstClipLayer =
          this.isLayerClipped(styleLayer) &&
          this._clipLayerIndices.some((c) => i < c);
        this.placement.updateLayerOpacities(
          styleLayer,
          layerTiles[makeFQID(styleLayer.source, styleLayer.scope)],
          i,
          checkAgainstClipLayer ? replacementSource : null
        );
      }
    }
    ...
    for (const layerId of this._mergedOrder) {
      ...
      for (const tile of tiles) {
        ...
        if (
          ...
          styleLayer.fqid === symbolBucket.layerIds[0]
        ) {
          opacityChanged =
            symbolBucket.updateOcclusionOpacities(
              painter.context,
              painter.symbolParams,
              painter._dt
            ) || opacityChanged;
        }
      }
  ...
  addImport(
    ...
    if (index !== -1) {
      this.fire(
        new ErrorEvent(
          new Error(
            `Import with id '${importSpec.id}' already exists in the map's style.`
          )
        )
      );
      return;
  ...
  updateImport(
    importId: string,
    importSpecification: ImportSpecification | string
  ): Style {
    this._checkLoaded();
    ...
    if (
      importSpecification.url &&
      importSpecification.url !== imports[index].url
    ) {
  ...
  setImportData(
    importId: string,
    stylesheet?: StyleSpecification | null
  ): Style {
    this._checkLoaded();
  ...
  setImportConfig(
    ...
    const fragment = this.fragments[index];
    const schema =
      fragment.style.stylesheet && fragment.style.stylesheet.schema;
  ...
  getImportIndex(importId: string): number {
    ...
    if (index === -1) {
      this.fire(
        new ErrorEvent(
          new Error(
            `Import '${importId}' does not exist in the map's style and cannot be updated.`
          )
        )
      );
    }
  ...
  _isSourceCacheLoaded(source: string): boolean {
    ...
    if (sourceCaches.length === 0) {
      this.fire(
        new ErrorEvent(new Error(`There is no source with ID '${source}'`))
      );
      return false;
    }
  ...
  isLayerClipped(layer: StyleLayer, source?: Source | null): boolean {
    ...
    if (this._clipLayerIndices.length === 0 && layer.type !== "fill-extrusion")
      return false;
    const isFillExtrusion =
      layer.type === "fill-extrusion" && layer.sourceLayer === "building";
    ...
    for (const i of this._clipLayerIndices) {
      ...
      const clipLayer: ClipStyleLayer = this._mergedLayers[
        this._mergedOrder[i]
      ] as any;
      ...
      for (const extra of clipLayer.layout.get("clip-layer-types"))
        extraLayersToBeClipped.push(
          extra === "model"
            ? LayerTypeMask.Model
            : extra === "symbol"
            ? LayerTypeMask.Symbol
            : LayerTypeMask.FillExtrusion
        );

      for (const mask of extraLayersToBeClipped)
        if (layerMask & mask) return true;
    }
