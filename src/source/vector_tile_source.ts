...
class VectorTileSource extends Evented<SourceEvents> implements ISource {
  ...
  load(callback?: Callback<undefined>) {
    ...
    const language = Array.isArray(this.map._language)
      ? this.map._language.join()
      : this.map._language;
    const worldview = this.map._worldview;
    this._tileJSONRequest = loadTileJSON(
      this._options,
      ...
      (err, tileJSON) => {
        ...
        if (err) {
          if (language)
            console.warn(
              `Ensure that your requested language string is a valid BCP-47 code or list of codes. Found: ${language}`
            );
          if (worldview && worldview.length !== 2)
            console.warn(
              `Requested worldview strings must be a valid ISO alpha-2 code. Found: ${worldview}`
            );
  ...
  loadTile(tile: Tile, callback: Callback<undefined>) {
    ...
    const lutForScope = this.map.style
