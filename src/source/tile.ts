...
class Tile {
  ...
  upload(context: Context) {
    ...
    if (atlas && !atlas.uploaded) {
      ...
      this.imageAtlasTexture = new Texture(context, atlas.image, gl.RGBA, {
        useMipmap: hasPattern,
      });
  ...
  prepare(
    ...
    if (
      ...
      brightness &&
      Math.abs(this._lastUpdatedBrightness - brightness) < 0.001
    ) {
      return;
    }
  ...
  queryRenderedFeatures(
    ...
    if (
      !this.latestFeatureIndex ||
      !(this.latestFeatureIndex.rawTileData || this.latestFeatureIndex.is3DTile)
    )
      return {};
  ...
  bucketsLoaded(): boolean {
    for (const id in this.buckets) {
      if (this.buckets[id].uploadPending()) return false;
    }
  ...
  updateBuckets(
    states: LayerFeatureStates | null | undefined,
    painter: Painter
  ) {
    ...
    for (const id in this.buckets) {
      ...
      if (bucket instanceof LineBucket || bucket instanceof FillBucket) {
        const sourceCache = painter.style.getOwnSourceCache(
  ...
  symbolFadeFinished(): boolean {
    return (
      !this.symbolFadeHoldUntil || this.symbolFadeHoldUntil < browser.now()
    );
  }
  ...
  _makeGlobeTileDebugBuffers(context: Context, transform: Transform) {
    ...
    if (phase > 0.0) {
      ...
      worldToECEFMatrix = mat4.invert(
        new Float64Array(16),
        transform.globeMatrix
      );
    }

    this._makeGlobeTileDebugBorderBuffer(
      context,
      id,
      transform,
      normalizationMatrix,
    ...
    this._makeGlobeTileDebugTextBuffer(
      context,
      id,
      transform,
      normalizationMatrix,
  ...
  _globePoint(
    ...
    if (worldToECEFMatrix) {
      ...
      vec3.transformMat4(
        mercatorPos as [number, number, number],
        mercatorPos as [number, number, number],
        worldToECEFMatrix
      );
