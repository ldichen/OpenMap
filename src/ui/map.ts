...
import {
  LivePerformanceMarkers,
  LivePerformanceUtils,
} from "../util/live_performance";
...
export class Map extends Camera {
  ...
  constructor(options: MapOptions) {
    ...
    this._colorChange = options.colorChange;
    this._baseMapColor = options.baseMapColor;
    ...
    if (typeof options.container === "string") {
      ...
      if (container) {
        ...
        throw new Error(
          `Container '${options.container.toString()}' not found.`
        );
      }
   ...
   */
  setProjection(
    projection?: ProjectionSpecification | null | undefined | string
  ): this {
    ...
    return this._prioritizeAndUpdateProjection(
      projection,
  ...
  _updateProjectionTransition() {
    ...
    if (projection === "globe" && tr.zoom >= GLOBE_ZOOM_THRESHOLD_MAX) {
      ...
      projection === "mercator" &&
      tr.zoom < GLOBE_ZOOM_THRESHOLD_MAX
    ) {
  ...
  _prioritizeAndUpdateProjection(
    ...
    //  3. mercator (fallback)
    const prioritizedProjection = explicitProjection ||
  ...
  _updateProjection(projection: ProjectionSpecification): this {
    ...
    if (
      ...
      this.transform.zoom >= GLOBE_ZOOM_THRESHOLD_MAX
    ) {
  ...
  isZooming(): boolean {
    return (
      this._zooming || (this.handlers && this.handlers.isZooming()) || false
    );
  }
  ...
  _createDelegatedListener<T extends MapEventType>(
    type: T,
    layers: Array<string>,
    listener: Listener<T>
  ): DelegatedListener {
    if (type === "mouseenter" || type === "mouseover") {
      ...
      const mousemove = (e: MapMouseEvent) => {
        ...
        const features = filteredLayers.length
          ? this.queryRenderedFeatures(e.point, { layers: filteredLayers })
          : [];
        if (!features.length) {
          mousein = false;
        } else if (!mousein) {
  ...
  on<T extends MapEventType>(
    type: T,
    layerIds: string | string[] | Listener<T>,
    listener?: Listener<T>
  ): this {
  ...
  once<T extends MapEventType>(
    type: T,
    layerIds: string | string[]
  ): Promise<MapEventOf<T>>;
  once<T extends MapEventType>(
    type: T,
    layerIds: string | string[],
    listener: Listener<T>
  ...
  off<T extends MapEventType>(
    ...
    const removeDelegatedListeners = (listeners: Array<DelegatedListener>) => {
      for (let i = 0; i < listeners.length; i++) {
        ...
        if (
          ...
          areLayerIdsEqual(delegatedListener.layers, uniqLayerIds)
        ) {
          for (const event in delegatedListener.delegates) {
    ...
    const delegatedListeners = this._delegatedListeners
  ...
  queryRenderedFeatures(
    ...
    geometry = geometry || [
      [0, 0] as PointLike,
      [this.transform.width, this.transform.height] as PointLike,
    ];
  ...
  isPointOnSurface(point: PointLike): boolean {
    ...
    if (name !== "globe" && name !== "mercator") {
      warnOnce(
        `${name} projection does not support isPointOnSurface, this API may behave unexpectedly.`
      );
    }
      ...
      this.style._diffStyle(
        ...
        (
          ...
          if (e) {
            ...
            warnOnce(
              `Unable to perform style diff: ${String(
                e.message || e.error || e
  ...
  _isValidId(id?: string | null): boolean {
    ...
    if (isFQID(id)) {
      this.fire(
        new ErrorEvent(new Error(`IDs can't contain special symbols: "${id}".`))
      );
      return false;
    }
  ...
  loadImage(
    url: string,
    callback: Callback<ImageBitmap | HTMLImageElement | ImageData>
  ) {
    ...
    getImage(
      ...
      (err, img) => {
        callback(
          err,
          img instanceof HTMLImageElement ? browser.getImageData(img) : img
        );
      }
    );
  }
  ...
  updateImport(
    ...
    if (
      typeof importSpecification !== "string" &&
      importSpecification.id !== importId
    ) {
  ...
  _queryFogOpacity(lnglat: LngLatLike): number {
    ...
    return this.style.fog.getOpacityAtLatLng(
   ...
   */
  setFeatureState(
    feature: FeatureSelector | GeoJSONFeature,
    state: FeatureState
  ): this {
  ...
  _updateContainerDimensions() {
    ...
    while (el && (!transformScaleWidth || !transformScaleHeight)) {
      ...
      if (transformMatrix && transformMatrix !== "none") {
        transformValues = transformMatrix
          .match(/matrix.*\((.+)\)/)[1]
          .split(", ");
        if (
          transformValues[0] &&
    ...
    this._containerWidth = transformScaleWidth
      ? Math.abs(width / transformScaleWidth)
      : width;
    this._containerHeight = transformScaleHeight
      ? Math.abs(height / transformScaleHeight)
      : height;
  }
  ...
  _render(paintStartTimeStamp: number) {
    ...
    if (updatePlacementResult) {
      ...
      this._occlusionOpacityChanged =
        updatePlacementResult.occlusionQueryBasedOpacityChanged;
    }
    ...
    if (gpuTimer) {
      ...
      setTimeout(() => {
        const renderGPUTime =
          gl.getQueryParameter(gpuTimer, gl.QUERY_RESULT) / (1000 * 1000);
    ...
    if (this.listens("gpu-timing-deferred-render")) {
      const deferredRenderQueries =
        this.painter.collectDeferredRenderGpuQueries();

      setTimeout(() => {
        const gpuTime = this.painter.queryGpuTimeDeferredRender(
          deferredRenderQueries
        );
    ...
    if (
      ...
      if (willIdle) {
        ...
        averageElevationChanged = this._updateAverageElevation(
          frameStartTime,
          true
        );
      }
      ...
      if (averageElevationChanged) {
        ...
        if (willIdle) {
          ...
          if (this.speedIndexTiming) {
            ...
            this.fire(
              new Event("speedindexcompleted", { speedIndex: speedIndexNumber })
            );
  ...
  _updateAverageElevation(
    ...
    const timeoutElapsed =
      ignoreTimeout ||
      timeStamp - this._averageElevationLastSampledAt >
        AVERAGE_ELEVATION_SAMPLING_INTERVAL;

    if (
      exaggerationChanged ||
      (timeoutElapsed && !this._averageElevation.isEasing(timeStamp))
    ) {
  ...
  _calculateSpeedIndex(): number {
    ...
    return this._canvasPixelComparison(
      read(finalFrame),
      canvasCopyInstances.canvasCopies.map(read),
      canvasCopyInstances.timeStamps
    );
  }

  _canvasPixelComparison(
  ...
  _preloadTiles(transform: Transform | Array<Transform>): this {
    const sourceCaches: Array<SourceCache> = this.style
      ? this.style.getSourceCaches()
      : [];
    asyncAll(
      sourceCaches,
      (sourceCache, done) => sourceCache._preloadTiles(transform, done),
      () => {
        this.triggerRepaint();
      }
    );
  ...
  _occlusionCriteriaSatisfied(): boolean {
    const occluderLayers =
      this.style &&
      (this.style.has3DLayers() ||
        (this.style.terrain && !this.style.disableElevatedTerrain));
    if (
      this.style &&
      ...
      return (
        this._frameId - this._lastDirtyFrameId >
        eps + factor * this.painter.symbolParams.occlusionQueryFrameWindow
      );
    } else {
      return true;
    }
