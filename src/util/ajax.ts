...
import { modifyImageBitmap } from "../openMapgl/customBaseMap";
...
function makeFetchRequest(
  ...
  const validateOrFetch = (
    ...
    fetch(request)
      .then((response) => {
        if (response.ok) {
          const cacheableResponse = cacheIgnoringSearch
            ? response.clone()
            : null;
  ...
  const finishRequest = (
    response: Response,
    cacheableResponse?: Response | null,
...
function arrayBufferToImage(
  ...
  img.onload = () => {
    console.log("arrayBufferToImage");
    callback(null, img);
...
function arrayBufferToImageBitmap(
  ...
  createImageBitmap(blob)
    .then((imgBitmap) => {
      if (map != undefined) {
        modifyImageBitmap(imgBitmap, map._baseMapColor)
          .then((newImageBitmap) => {
            callback(null, newImageBitmap);
          })
          .catch((e) => {
            console.log("原图", e.message);
        ...
        if (self.createImageBitmap) {
          if (map != undefined) {
            arrayBufferToImageBitmap(
              data,
              (err, imgBitmap) =>
                callback(err, imgBitmap, cacheControl, expires),
              map
            );
          }
          // else {
          //   arrayBufferToImageBitmap(data, (err, imgBitmap) =>
          //     callback(err, imgBitmap, cacheControl, expires)
          //   );
          // }
        } else {
          arrayBufferToImage(data, (err, img) =>
            callback(err, img, cacheControl, expires)
          );
        }
      }
    }
...
export const getVideo = function (
  urls: Array<string>,
  callback: Callback<HTMLVideoElement>
): Cancelable {
