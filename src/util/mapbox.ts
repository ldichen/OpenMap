...
export class RequestManager {
  ...
  constructor(
    transformRequestFn?: RequestTransformFunction | null,
    customAccessToken?: string | null,
    silenceAuthErrors?: boolean | null
  ) {
  ...
  canonicalizeTileURL(url: string, removeAccessToken: boolean): string {
    ...
    if (
      !urlObject.path.match(/^(\/v4\/|\/(raster|rasterarrays)\/v1\/)/) ||
...
export class PerformanceEvent extends TelemetryEvent {
  ...
  postPerformanceEvent(
    customAccessToken: string | null | undefined,
...
export class TurnstileEvent extends TelemetryEvent {
  ...
  processRequests(customAccessToken?: string | null) {
    ...
    if (this.eventData.lastSuccess) {
      ...
      const daysElapsed =
        (nextUpdate - this.eventData.lastSuccess) / (24 * 60 * 60 * 1000);
      dueForEvent =
        dueForEvent ||
        daysElapsed >= 1 ||
        daysElapsed < -1 ||
...
export const postTurnstileEvent: (
  tileUrls: Array<string>,
  customAccessToken?: string | null | undefined
) => void = turnstileEvent_.postTurnstileEvent.bind(turnstileEvent_);
...
export const postStyleLoadEvent: (
  arg1: string | null | undefined,
  arg2: StyleLoadEventInput
) => void = styleLoadEvent.postStyleLoadEvent.bind(styleLoadEvent);

export const performanceEvent_: PerformanceEvent = new PerformanceEvent();
export const postPerformanceEvent: (
  arg1: string | null | undefined,
  arg2: LivePerformanceData
) => void = performanceEvent_.postPerformanceEvent.bind(performanceEvent_);
